...
| コマンド | 説明 |
| --- | --- |
| `npm run dev` | 開発サーバー起動（Vite、フォアグラウンド） |
| `npm run dev:bg` | 開発サーバーをバックグラウンドで起動（PID 管理付き） |
| `npm run dev:status` | 開発サーバーの稼働状態を確認 |
| `npm run dev:stop` | バックグラウンドの開発サーバーを停止 |
...
```bash
npm run dev        # フォアグラウンドで起動
npm run dev:bg     # バックグラウンドで起動（推奨）
npm run dev:status # 稼働状態を確認
npm run dev:stop   # 停止
```
