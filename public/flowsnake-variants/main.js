...
function draw() {
  ...
  const depth = Math.max(1, Math.min(5, params.depth | 0));
  const axiom = 'A';
  const rules = {
    A: 'A-B--B+A++AA+B-',
    B: '+A-BB--B-A++A+B',
...
gui.add(params, 'depth', 1, 5, 1).onChange(onChange);
gui.add(params, 'angle', 30, 90, 0.5).onChange(onChange);
